package il.co.topq.systems.report.business.summary
{
	...
	public class GetTotalNumberOfTestsDelegate  implements  IResponder
	{
		private var responder:IResponder;
		private var model: ReportModelLocator = ReportModelLocator.getInstance();
		
		public function GetTotalNumberOfTestsDelegate(){
			this.responder = this;
		}
		
		public function getTotalNumberOfTests(scenarioQuery:XML):void{
			if (scenarioQuery != null){
				var service:HTTPService = ServiceLocator.getInstance().getHTTPService("countTestsByScenarioQuery");
		...
		{
			var testsStatistics:XML = new XML(data.message.body);
			model.summaryDataVO.totalTests = testsStatistics.total;
			model.summaryDataVO.setTestStatistics(testsStatistics);
		}
